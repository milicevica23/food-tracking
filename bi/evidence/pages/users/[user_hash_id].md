...
with cte as (
    ...
    from motherduck.food_table_ingredients_flat
    where user_hash_id like '${params.user_hash_id}'
)
select * from cte
...
select
  allergen,
  count(allergen) as count_allergen,
  avg(state_point) as average_state_point,
  count(state_point) as count_state_point,
  avg(sleep_point) as average_sleep_point,
  count(sleep_point) as count_sleep_point,
...
<BarChart 
    data={food_table_allergens_flat_filtered_average_points}
    x=allergen
    y=count_allergen
    title="Sum of Occurrences per Allergen"
/>
